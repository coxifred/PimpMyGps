...
public class Log {
	...
	{
		return new Log(aLog.getDate(),aLog.getS_date(),aLog.getThread(),aLog.getSeverity(),aLog.getFrom(),aLog.getMessage());
	}
		...
		}else
		{
			...
			for (Log originalLog:entries)
			{
				...
				if ( include != null && ! "".equals(include))					
				{
					...
					{
					includeB=true; aLog.setS_date(aLog.getS_date().replaceAll(include, "<font color=red>" + include + "</font>"));
					}
					if ( aLog.getMessage().contains(include))
					{
					includeB=true; aLog.setMessage(aLog.getMessage().replaceAll(include, "<font color=#fc00f8>" + include + "</font>"));
					}
					if ( aLog.getSeverity().contains(include))
					{
					includeB=true; aLog.setSeverity(aLog.getSeverity().replaceAll(include, "<font color=red>" + include + "</font>"));
					}
					if ( aLog.getThread().contains(include))
					{
					includeB=true; aLog.setThread(aLog.getThread().replaceAll(include, "<font color=red>" + include + "</font>"));
					}
					if ( aLog.getFrom().contains(include))
					{
					includeB=true; aLog.setFrom(aLog.getFrom().replaceAll(include, "<font color=red>" + include + "</font>"));
					}
				}else
				{
					includeB=true;
